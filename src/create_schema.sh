...
# Set parameters
while getopts :hU:H:p:s:d:w:n:i:m:ETB flag
do
    case "${flag}" in
        h) echo "$usage"; exit;;
        U) user="${OPTARG}";;
...
if [ ! "${user}" ] || [ ! "${host}" ] || [ ! "${port}" ] || [ ! "${schema}" ] || [ ! "${database}" ] || [ ! "${password}" ] || [ ! "${tidname}" ] || [ ! "${interlisfile}" ] || [ ! "${interlismodel}" ]; then
